...
<body class="hold-transition skin-purple sidebar-mini">
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										...
										<button type="button" class="btn btn-default" title="产品选择" onclick="selectProduct()">
											<i class="fa fa-check-circle"></i> 产品选择
										</button>
									...
									<c:forEach items="${pageInfo.list}" var="orders">
										...
										<tr>
											<td class="text-center">
												<input name="ids" id="pd" type="radio" value="${orders.id}" onclick="checkOnclick(this)">
											</td>
											<td>${orders.id }</td>
											<td>${orders.orderNum }</td>
											<td>
												<c:if test="${orders.product.productName == null}"><p style="color: rosybrown">暂未选择产品！</p></c:if>
												<c:if test="${orders.product.productName !=null}">${orders.product.productName }</c:if>
											</td>
											...
											<td class="text-center">
												...
												<button type="button" class="btn bg-olive btn-xs" onclick="location.href='${pageContext.request.contextPath}/orders/findByIdAddTraveller.do?id=${orders.id}'">添加旅客</button>
												<button type="button" class="btn bg-olive btn-xs" onclick="location.href='${pageContext.request.contextPath}/orders/findByIdEdit.do?id=${orders.id}'">编辑</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
					...
					<div class="box-tools pull-right">
						<ul class="pagination">
							...
							<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
								<a href="${pageContext.request.contextPath}/orders/findAll.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}">下一页</a></li>
							<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
	...
	<script
		src="${pageContext.request.contextPath}/plugins/adminLTE/js/app.min(1).js
"></script>
	<script
	...
	<script>
		...
		function selectProduct() {
			...
			if ($("input[name='ids']").is(':checked')){
				$("input[name='ids']:checked").each(function () {
					ordersId.push($(this).val());
					window.location.href="${pageContext.request.contextPath}/orders/findByIdselectProduct.do?ordersId="+ordersId;
				});
			} else {
				alert("未选中！");
			}
