...
<body class="hold-transition skin-purple sidebar-mini">
			...
			<form action="" method="post">
				...
				<input type="hidden" name="productId" value="">
								...
								<c:forEach items="${memberList}" var="member">
									<tr>
										<td class="text-center">
											<input name="ids" type="radio" value="${member.id}" onclick="checkOnclick(this)">
										</td>
										<td>${member.id}</td>
										<td>${member.name}</td>
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								<li <c:if test="${pageInfo.pageNum == 1}">class="disabled" </c:if>>
									<a href="${pageContext.request.contextPath}/orders/selectMember.do?page=1&size=${pageInfo.pageSize}" aria-label="Previous">首页</a>
								</li>
								<li <c:if test="${pageInfo.pageNum == 1}">class="disabled" </c:if>>
									<a href="${pageContext.request.contextPath}/orders/selectMember.do?page=${pageInfo.pageNum-1}&size=${pageInfo.pageSize}">上一页</a>
								</li>
								<c:if test="${pageInfo.pageNum<=pageCount}">
									<c:forEach begin="${pageInfo.pageNum}" end="${pageInfo.pageNum<=pageCount-5?pageInfo.pageNum+5:pageCount}" var="pageNum">
										<li <c:if test="${pageInfo.pageNum == pageNum}">class="active" </c:if>><%--显示选中的页数--%>
											<a href="${pageContext.request.contextPath}/orders/selectMember.do?page=${pageNum}&size=${pageInfo.pageSize}">${pageNum}</a>
										</li>
									</c:forEach>
								</c:if>
								<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
									<a href="${pageContext.request.contextPath}/orders/selectMember.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}">下一页</a></li>
								<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
									<a href="${pageContext.request.contextPath}/orders/selectMember.do?page=${pageInfo.pages}&size=${pageInfo.pageSize}" aria-label="Next">尾页</a>
								</li>
							</ul>
						</div>
					...
					<div class="box-tools text-center">
						<button type="button" class="btn bg-maroon" id="saveId" onclick="selectP()">选择</button>
						<a href="${pageContext.request.contextPath}/pages/orders-add.jsp">
	...
	<script>
		...
		function changePageSizes() {
			...
			//向服务器发送请求，改变每页显示条数
			location.href = "${pageContext.request.contextPath}/orders/selectMember.do?page=1&size="+ pageSize;
		...
		function selectP() {
			...
			if ($("input[name='ids']").is(':checked')){
				...
				$("input[name='ids']:checked").each(function () {
					arrId.push($(this).val());
					window.location.href="${pageContext.request.contextPath}/orders/getMemberById.do?mid="+arrId;
				})
			} else {
				alert("未选中！");
			}
