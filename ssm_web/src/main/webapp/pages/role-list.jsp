					...
					<div class="box-body">
						...
						<div class="table-box">
							...
							<table id="dataList"
								...
								<thead>
									<tr>
										<th style="padding-right: 0px" class="text-center">选择角色</th>
								...
								<tbody>
									...
									<c:forEach items="${pageInfo.list}" var="role">
										<tr>
											<td class="text-center"><input name="ids" type="radio" value="${role.id }"></td>
											<td>${role.id }</td>
											<td>${role.roleName }</td>
											<td>${role.roleDesc }</td>																				
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								...
								<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
									<a href="${pageContext.request.contextPath}/role/findAll.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}">下一页</a></li>
								<li <c:if test="${pageInfo.pageNum == pageCount}">class="disabled" </c:if>>
			...
			function deleteRole() {
				...
				if ($("input[name='ids']").is(':checked')){
					$("input[name='ids']:checked").each(function () {
						arrId.push($(this).val());
						$.post("${pageContext.request.contextPath}/role/delete.do?id="+arrId,function (data) {
							alert("删除成功！");
