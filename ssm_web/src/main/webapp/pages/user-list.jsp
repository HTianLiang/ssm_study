	...
	<div class="wrapper">
					...
					<div class="box-body">
						...
						<div class="table-box">
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										...
										<button type="button" class="btn btn-default" title="状态开启" onclick="openStatu()">
											<i class="fa fa-check-circle"></i> 开启
										</button>
										<button type="button" class="btn btn-default" title="状态关闭" onclick="closeStatu()">
							...
							<table id="dataList"
								...
								<thead>
									<tr>
										<th class="text-center" style="padding-right: 0px">选择用户</th>
									...
									<c:forEach items="${pageInfo.list}" var="user">
										<tr>
											<td class="text-center">
												<input name="Ustatu" type="radio" value="${user.statusStr }">
											</td>
											<td>${user.id }</td>
											<td>${user.username }</td>
											<td>${user.email }</td>
					...
					<div class="box-footer">
                        ...
                        <div class="box-tools pull-right">
                            <ul class="pagination">
                                ...
                                <li>
                                    <a href="${pageContext.request.contextPath}/user/findAll.do?page=${pageInfo.pageNum+1}&size=${pageInfo.pageSize}">下一页</a></li>
                                <li>
		...
		<script>
			...
			function deleteUser(id) {
				/*var arrId = [];
				//判断name=Uid的input框是否选中，若选中则将其value值追加到数组arrId中
				$("input[name='Uid']:checked").each(function () {
					arrId.push($(this).val());
				})*/
				if (confirm("你确定要删除该用户吗？")){
					$.post("${pageContext.request.contextPath}/user/delete.do",{"id":id},function (data) {
						alert("用户删除成功！");
			...
			function openStatu() {
				var arrStatu = [];
				if ($("input[name='Ustatu']").is(':checked')){

					//判断name=Uid的input框是否选中，若选中则将其value值追加到数组arrId中
					$("input[name='Ustatu']:checked").each(function () {
						arrStatu.push($(this).val());

						if (arrStatu=="开启"){
							alert("该项已是开启状态,请选择未开启状态的选项！")
						}else {
							//获取选中行的用户id id在tr标签下td标签的第二列
							var Uid = $(this).parents('tr').children().eq(1).text();

							$.post("${pageContext.request.contextPath}/user/open.do?id="+Uid,function (data) {
								window.location.reload();
							})
						}
					})
			...
			function closeStatu() {
				...
				if ($("input[name='Ustatu']").is(':checked')){
					...
					$("input[name='Ustatu']:checked").each(function () {
						...
						if (arrStatu=="未开启"){
							alert("该项已是未开启状态,请选择开启状态的选项！")
						}else {
							//获取选中行的用户id id在tr标签下td标签的第二列
							var Uid = $(this).parents('tr').children().eq(1).text();

							$.post("${pageContext.request.contextPath}/user/close.do?id="+Uid,function (data) {
								window.location.reload();
							})
						}
					})
